...
/*   Updated: 2026/05/10 20:08:36 by hermesrolle      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_col_vue_lower_bound(t_all *all, t_coor coor)
{
	...
	while (++i <= all->tab_size)
	{
		...
		h[1] = all->map[all->tab_size - i + 1][coor.x];
		if (h[1] == 0 && !first_zero[1])
		{
			h[1] = all->heatmap[all->tab_size - i + 1][coor.x];
			first_zero[1] = 1;
		}
		if (h[1] > max[1])
		{
	...
	return ((vue[0] <= all->map[0][coor.x]) /* && (vue[1] <= all->map[all->tab_size + 2][coor.x])*/ );
}
...
int	check_line_vue_lower_bound(t_all *all, t_coor coor)
{
	...
	while (++i <= all->tab_size)
	{
		...
		h[1] = all->map[coor.y][all->tab_size - i + 1];
		if (h[1] == 0 && !first_zero[1])
		{
			h[1] = all->heatmap[coor.y][all->tab_size - i + 1];
			first_zero[1] = 1;
		}
		if (h[1] > max[1])
		{
	...
	return ((vue[0] <= all->map[coor.y][0]) /* && (vue[1] <= all->map[coor.y][all->tab_size + 2])*/);
}
